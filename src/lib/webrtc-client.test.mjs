...
import { createCallClient } from './webrtc-client.ts';
...
test('destroying before the initial join starts does not acquire socket membership', async () => {
  const { client, socket, track } = harness();
  const joining = client.join('call-1');
  const rejected = assert.rejects(joining, /cancelled/);
  await client.destroy();
  await rejected;
  assert.equal(track.readyState, 'ended');
  assert.equal(socket.outgoing.filter((entry) => entry.event === 'call:join').length, 0);
  assert.equal(socket.listenerCount('webrtc:offer'), 0);
});

test('a replacement client waits until the previous pending join has left the shared socket', async () => {
  const { client, socket, track } = harness({ deferJoin: true });
  const joining = client.join('call-1');
  const rejected = assert.rejects(joining, /cancelled/);
  await drain();
  const firstJoinAck = socket.pendingJoin;
  const destroying = client.destroy();
  assert.equal(track.readyState, 'ended', 'capture must stop without waiting for the join ACK');
  const replacementTrack = { readyState: 'live', stop() { this.readyState = 'ended'; } };
  const replacement = createCallClient({ socket, localStream: new MediaStream([replacementTrack]) });
  const replacementJoining = replacement.join('call-2');
  await drain();
  assert.equal(socket.outgoing.filter((entry) => entry.event === 'call:join').length, 1);
  firstJoinAck();
  await destroying;
  await rejected;
  await drain();
  assert.deepEqual(socket.outgoing.map((entry) => [entry.event, entry.payload.callId]), [
    ['call:join', 'call-1'], ['call:leave', 'call-1'], ['call:join', 'call-2'],
  ]);
  socket.pendingJoin();
  await replacementJoining;
  await replacement.destroy();
  assert.equal(replacementTrack.readyState, 'ended');
  assert.equal(socket.connected, true, 'room cleanup must preserve the global notification socket');
  assert.equal(socket.listenerCount('webrtc:offer'), 0);
});
